...
// --------------------------------------
// 图片依次滑动（左右）
function lunbof(img,cir,left1,right1,stop,width,classmaneo,classnamet){
	// 图片img
	var img=img;
	// 滑动点
	var cir=cir;
	...
	function fun(){
		...
		for (var i = 0; i < img.length; i++) {
			cir[i].style.background="#6E6E6E";
		};
		cir[next].style.background="#E6185F";
		n=next;
	};
	for (var i = 0; i < cir.length; i++) {
		cir[i].index=i;
		...
		cir[i].onclick=function(){
			...
			cir[this.index].style.background="#E6185F";
				if(n>this.index){
...
function log(pplt1,pprt1,inner1){
	var pplt1=pplt1;
	var pprt1=pprt1;
	var inner1=inner1;
	var n=0;
 	var next=0
 		pplt1.onclick=function(){
 			next=n+1;
 			if(next>inner1.length-2){
				next=0
			}
			inner1[n].style.left="0px"
			inner1[next].style.left="200px"
			animate(inner1[n],{left:-200},600);
			animate(inner1[next],{left:0},600);

 			next=n;
 		}
		pprt1.onclick=function(){
		next=n-1;
		if(next<0){
			next=inner1.length-2
		}
		inner1[n].style.left="0px"
		inner1[next].style.left="-200px"
		animate(inner1[n],{left:200},600);
